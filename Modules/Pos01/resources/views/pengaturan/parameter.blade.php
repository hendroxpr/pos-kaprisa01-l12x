            ...
            <table id="data1" class="table table-bordered table-striped table-hover" style="width: 100%">
                <thead>
                    <tr>
                        ...
                        <th style="width:50px">Angsuran (x)</th>							
                        <th style="width:100px">% Jasa Hutang Barang</th>
                        ...
                        <div class="modal-body" id="bodyAdd" name="bodyAdd">                        
                            ...
                            <div class="row">
                                <div class="col-md-4 mt-1" align="right">									
                                    <h6 class="mt-2">Angsuran (x) *)</h6>
                                </div>
                                <div class="col-md-8"> 
                                    <input name="xangsuran1" id="xangsuran1" class="w3-input w3-border" maxlength="5" type="search" placeholder="Xangsuran" autofocus value="{{ old('xangsuran1') }}" onkeypress="return hanyaAngka(event)" required>
                            ...
                            <div class="row">
                                <div class="col-md-4 mt-1" align="right">									
                                    <h6 class="mt-2">% Jasa Hutang brg *)</h6>
                                </div>
                                <div class="col-md-8">                                
                                    <input name="jasahutangbarang1" id="jasahutangbarang1" class="w3-input w3-border" maxlength="7" type="search" placeholder="Jasahutangbarang" value="{{ old('jasahutangbarang1') }}" onkeypress="return hanyaAngkaTitik(event)" required>
                                </div>								  
                            </div>
                            <div class="row">
                                <div class="col-md-4 mt-1" align="right">									
...
$(document).ready(function(){
    ...
    setTimeout(() => {
     ...
     function tampil_data1(){
        ...
        return $('#data1').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax   : `{{route('pos01.pengaturan.parameter_show')}}`,
            columns: [
                ...
                {  "data": 'DT_RowIndex',
                    orderable: false, 
                    searchable: false },
                { data: 'xangsuran', name: 'xangsuran' },
                { data: 'jasahutangbarang', name: 'jasahutangbarang' },
    ...
    function data_simpan(){
        var id1=$('#id1').val();			
        var xangsuran1=$('#xangsuran1').val();
        var jasahutangbarang1=$('#jasahutangbarang1').val();
        var keterangan1=$('#keterangan1').val();
        
        let formData = new FormData();
            formData.append('id1', id1);
            formData.append('xangsuran1', xangsuran1);
            formData.append('jasahutangbarang1', jasahutangbarang1);
            formData.append('keterangan1', keterangan1);
          
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('pos01.pengaturan.parameter_create')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
            ...
            error : function(formData){                    
                swalgagaltambah(jasahutangbarang1);                 
                }
        });
    ...
    function data_edit(idx){
        var id1=idx;			
            $.ajax({
		        type  : 'get',
		        url   : `{{ url('pos01/pengaturan/parameteredit')}}/${id1}`,
		        async : false,
		        dataType : 'json',	
		        ...
		        success : function(data){
                    ...
                    for(i=0; i<resultData.length; i++){
                        ...
                        $('#xangsuran1').val(resultData[i].xangsuran);
                        $('#jasahutangbarang1').val(resultData[i].jasahutangbarang);
    ...
    function data_hapus(){			
        ...
        $.ajax({
            type  : 'get',
            url   : '{{url('pos01/pengaturan/parameterdestroy')}}/'+id3,
            async : false,
            dataType : 'json',					
            success : function(data){
                tampil_dataTable();
    ...
    $("#xangsuran1").on('change',function(){
        
        if($("#xangsuran1").val()==''){
            $("#xangsuran1").val('0');
            var x = parseFloat($("#xangsuran1").val())
        }else{
            var x = parseFloat($("#xangsuran1").val())
        }
        
    });
    $("#xangsuran1").on('keydown',function(){
        if($("#xangsuran1").val()==''){
            $("#xangsuran1").val('0');
            var x = parseFloat($("#xangsuran1").val())
        }else{
    ...
    $("#jasahutangbarang1").on('change',function(){
        if($("#jasahutangbarang1").val()==''){
            $("#jasahutangbarang1").val('0');
        }
    });
    $("#jasahutangbarang1").on('keydown',function(){
        if($("#jasahutangbarang1").val()==''){
            $("#jasahutangbarang1").val('0');
        }
    });
