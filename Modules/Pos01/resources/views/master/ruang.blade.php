        ...
        <div class="row">
            <div class="col-md-6">
                <div class="row">
                    ...
                    <div class="col-md-7">
                        <select name="idseksi1" id="idseksi1" class="w3-input w3-border" value="{{ $idseksi }}"></select> 
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-3">
                    ...
                    <form class="form-horizontal" id="formSimpan" nama="formSimpan" action="" method="">
                        ...
                        <div class="modal-body" id="bodyAdd" name="bodyAdd">                        
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-8">                                
                                    <input name="kode1" id="kode1" class="w3-input w3-border" maxlength="7" type="search" placeholder="Kode" autofocus value="{{ old('kode1') }}" required>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-8">                                
                                    <input name="ruang1" id="ruang1" class="w3-input w3-border" maxlength="50" type="search" placeholder="ruang" value="{{ old('ruang1') }}" required>
                                </div>
                            </div> 
...
$(document).ready(function(){
    ...
    function tampil_data(){	
        $.ajax({
            type  : 'get',
            url   : '{{route('pos01.master.ruang_show')}}',
            async : false,
            dataType : 'json',
            ...
            success : function(data){
                ...
                for(i=0; i<resultData.length; i++){
                    ...
                    html += '<tr>'+
                            ...
                            '<td>'+resultData[i].ruang+'</td>'+
                            '<td>'+(resultData[i].keterangan ? resultData[i].keterangan : '')+'</td>'+
                            '<td style="text-align:center;">'+
                                '<a href="javascript:;" title="Edit Data"  class="btn btn-success btn-xs item_edit" data="'+resultData[i].id+'" data2="'+resultData[i].kode+'" data3="'+resultData[i].ruang+'" data4="'+resultData[i].keterangan+'"><i style="font-size:18px" class="fa">&#xf044;</i></a>'+ ' ' +
                                '<a href="javascript:;" title="Hapus Data"  class="btn btn-danger btn-xs item_hapus" ' +
                                    'data="'+resultData[i].id+'" data2="'+resultData[i].kode+'" data3="'+resultData[i].ruang+'" data4="'+resultData[i].keterangan+'" ' +
    ...
    //menampilkan combo seksi
    function tampil_listseksi(){				
        $.ajax({
            type: 'get',
            url   : '{{route('pos01.master.ruang_listseksi')}}',
            
            success: function(data){				    
                $("#idseksi1").html(data);
                $("#idseksi1").val({{ $idseksi }});

            }
        })                    
    }

    $('#idseksi1').on('change',function(){
        setTimeout(() => {
            kirimsyarat();	
        }, 500);
    });
    ...
    function kirimsyarat(){
        var idseksi1=$('#idseksi1').val();
        
        let formData = new FormData();
            formData.append('idseksi1', idseksi1);

        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('pos01.master.ruang_kirimsyarat')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    function data_simpan(){
        ...
        let formData = new FormData();
            ...
            formData.append('idseksi1', idseksi1);
            formData.append('ruang1', ruang1);
            formData.append('keterangan1', keterangan1);            
          
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('pos01.master.ruang_create')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    function data_edit(idx){
        var id1=idx;			
            ...
            $.ajax({
		        type  : 'get',
		        url   : `{{ url('pos01/master/ruangedit')}}/${id1}`,
		        async : false,
		        dataType : 'json',	
		        ...
		        success : function(data){
                    ...
                    for(i=0; i<resultData.length; i++){
						...
						$('#idseksi1').val(resultData[i].idseksi);
						$('#ruang1').val(resultData[i].ruang);
    ...
    function data_hapus(){			
        ...
        $.ajax({
            type  : 'get',
            url   : '{{url('pos01/master/ruangdelete')}}/'+id3,
            async : false,
            dataType : 'json',					
            success : function(data){
                tampil_data();
