        ...
        <div class="row">
            <div class="col-md-6">
                <div class="row">
                    ...
                    <div class="col-md-8">
                        <select name="idmenuutama1" id="idmenuutama1" class="w3-input w3-border" value="{{ $idmenuutama }}"></select> 
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-3 mt-2 text-right">
                    ...
                    <div class="col-md-8">
                        <select name="idaplikasi1" id="idaplikasi1" class="w3-input w3-border" value="{{ $idaplikasi }}"></select>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                    ...
                    <form class="form-horizontal" id="formSimpan" nama="formSimpan" action="" method="">
                        ...
                        <div class="modal-body" id="bodyAdd" name="bodyAdd">                        
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-8">                                
                                    <input name="link1" id="link1" class="w3-input w3-border" type="text" placeholder="Link" autofocus value="{{ old('link1') }}" required>
                                </div>								  
                            </div>
...
$(document).ready(function(){
    ...
    function tampil_listaplikasi(){				
        $.ajax({
            type: 'get',
            url   : '{{route('admin.menusub_listaplikasi')}}',
            
            success: function(data){				    
                $("#idaplikasi1").html(data);                
                $("#idaplikasi1").val({{ $idaplikasi }});                
            }
        })
    }
    
    //menampilkan combo menuutama
    function tampil_listmenuutama(){				
        $.ajax({
            type: 'get',
            url   : '{{route('admin.menusub_listmenuutama')}}',
            
            success: function(data){				    
                $("#idmenuutama1").html(data);                
                $("#idmenuutama1").val({{ $idmenuutama }});                
            }
        })                    
    }

    $('#idmenuutama').on('change',function(){
        setTimeout(() => {
            kirim_syarat();	
        }, 500);
    });

    $('#idaplikasi').on('change',function(){
        setTimeout(() => {
            kirim_syarat();	
        }, 500);	
    });
    
    function kirim_syarat(){
        var idmenuutama1=$('#idmenuutama1').val();
        var idaplikasi1=$('#idaplikasi1').val();
        
        let formData = new FormData();
            formData.append('idmenuutama1', idmenuutama1);
            formData.append('idaplikasi1', idaplikasi1);

        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('admin.menusub_kirimsyarat')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    function data_simpan(){
        ...
        var urutan1=$('#urutan1').val().replace(/[^,\d]/g, '').toString();        
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('admin.menusub_create')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    function data_hapus(){			
        ...
        $.ajax({
            type  : 'get',
            url   : '{{url('/admin/menusubdelete')}}/'+id3,
            async : false,
            dataType : 'json',					
            success : function(data){
                tampil_data();
